...
(* build: (int*int->int) * int -> Expr 
   ...
   when the depth is 0, randomly return X or Y
   else use a random number to decide current operator type
   (sin, cos, avg, multiplication, threshold)
   recursive calls to build sub expressions
*)

let rec build (rand,depth) = 
  if depth = 0 then (if (rand (0, 2)) = 0 then buildX()  else buildY())
...
(* build2: similar with build, except for using two more expression operators:
   sigmoid function, maximum of three input
 *)
let rec build2 (rand,depth) = 
  ...
  | 6   -> buildMaximum ((build (rand, depth-1)), (build (rand, depth-1)), (build (rand, depth-1)))
;;
...
let g1 () = (6,26,96) 
let g2 () = (3,19,34)
let g3 () = (11,79,93) (* doRandomGray2 *)

let c1 () = (4,2,1997) (* doRandomColor2 *)
let c2 () = (4,2,97) 
let c3 () = (9,18,18)
